...
function buildQuickstartPrompt(): GetPromptResult {
  const text = joinLines([
    ...
    '- `list_course_materials`: aggregate course module resources (files, pages, assignments, quizzes, discussions, and external links) before browsing manually.',
    '- `resolve_external_downloads`: resolve ExternalUrl/ExternalTool materials to candidate download links before any browser-only fallback.',
    ...
    '4. For course resource discovery, call `list_course_materials` first; when items are ExternalUrl/ExternalTool, run `resolve_external_downloads` before any browser fallback.',
    '5. For file requests, use folder tools to navigate structure first, then list/search files, and finally get download URLs when needed.',
    '6. Read tool results from `structuredContent` and combine them into a concise answer grouped by course and sorted by due date.',
    '7. Highlight overdue or at-risk work, include Canvas links when present, and suggest next actions or follow-up tools when data is missing.',
    '8. If no relevant data is found, state that clearly and recommend what the learner can try next (different filters, future check-ins, etc.).',
    '',
...
function buildFileAccessPrompt(args: {
  ...
  const text = joinLines([
    ...
    '2. For course resources, run `list_course_materials` first; if you see ExternalUrl/ExternalTool items, run `resolve_external_downloads` before browser fallback.',
    '3. For course files, use `list_courses` to get the course ID, then call `list_course_folders` to show the folder structure.',
    '4. Use `list_folder_files` to browse specific folders, or search directly with `list_course_files` using search_term or content_types filters.',
    '5. When the learner identifies a file of interest, use `get_file` to show detailed metadata (size, upload date, lock status).',
    '6. If the learner needs to download or view the file, call `get_file_download_url` to provide a temporary signed download link.',
    '7. Explain that download URLs expire after 10 minutes and should be used immediately, not saved for later.',
    '',
